...
export const registro = async (req, res) => {  // req es la solicitud y res es la respuesta
  try {
    const { nombre, apellido, usuario, email, password, role = 'user' } = req.body; //  extrae los datos del cuerpo de la solicitud y Definir un rol por defecto de 'user', si no se pasa 'role'
    
    if (role !== 'admin' && role !== 'user') {
      return res.status(400).json({ message: 'Rol inválido' });
    }

    const user = new Usuarios({ nombre, apellido, usuario, email, password, role }); // crea un nuevo usuario con los datos recibidos
